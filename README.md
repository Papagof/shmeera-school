...
Wire `sweep-expired-codes` (every minute) and `purge-retention` (daily) via `pg_cron` + `pg_net`, calling each function's URL with an `x-cron-secret` header matching `CRON_SECRET`.
...
- Chat attachment upload + virus/type scanning (SPEC.md §3.3.3, §5.4) — needs its own upload path and RLS policy, plus a scanning step before a message is marked delivered.
