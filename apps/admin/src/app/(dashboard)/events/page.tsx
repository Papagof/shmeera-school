...
export default async function EventsPage() {
  ...
  const [{ data: events, error }, { data: classes }] = await Promise.all([
    supabase
      .from("events")
      .select(
        "id, school_id, type, released_to_name, created_at, geo, students(full_name, class_id), staff:validated_by_staff_id(full_name)",
      )
  ...
  return (
    <div>
      <h1 className="text-lg font-semibold text-slate-900">Live drop-off / pickup feed</h1>
